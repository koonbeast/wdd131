...
let recipeCard = document.querySelector("#recipe-list")
...
function search(event) {
	...
	filteredRecipes.forEach(renderRecipe)
}
...
function recipesTemplate(recipe) {
	return `        
        ...
        <div class="recipe-card">
            ...
            <img class="recipe-image" src="${recipe.image}" alt="food item">

            <section class="recipe-card-contents">
				${recipe.tags.map(tag => `<button type="button">${tag}</button>`).join("")}
                ...
                >
                    <span aria-hidden="true" class="icon-star">${"⭐".repeat(recipe.rating)}</span>

                    <span aria-hidden="true" class="icon-star-empty">${"☆".repeat(5 - recipe.rating)}</span>
